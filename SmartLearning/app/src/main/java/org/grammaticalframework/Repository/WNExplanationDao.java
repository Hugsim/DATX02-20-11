...
public interface WNExplanationDao {
    ...
    @Query("SELECT * FROM " +
            "(SELECT * FROM WordNetExplanation_table WHERE function IN (:functions)) t1 " +
            "LEFT OUTER JOIN " +
            "(SELECT * FROM CheckedFunction_table WHERE langcode = :langcode AND checked_function IN (:functions)) t2 " +
